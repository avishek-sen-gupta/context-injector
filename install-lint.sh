...
cp "$PLUGIN_DIR/hooks/lint-check.sh" ~/.claude/plugins/context-injector/hooks/
chmod +x ~/.claude/plugins/context-injector/hooks/lint-check.sh

# --- install CLI ---
echo "Installing lint CLI..."
mkdir -p ~/.claude/plugins/context-injector/bin
cp "$PLUGIN_DIR/bin/lint" ~/.claude/plugins/context-injector/bin/
chmod +x ~/.claude/plugins/context-injector/bin/lint
...
if [ "$EDIT_WIRED" = "false" ]; then
  ...
  HOOK_ENTRY='{"matcher": "Edit", "hooks": [{"type": "command", "command": "~/.claude/plugins/context-injector/hooks/lint-check.sh"}]}'
...
if [ "$WRITE_WIRED" = "false" ]; then
  ...
  HOOK_ENTRY='{"matcher": "Write", "hooks": [{"type": "command", "command": "~/.claude/plugins/context-injector/hooks/lint-check.sh"}]}'
  jq --argjson entry "$HOOK_ENTRY" \
    '.hooks.PreToolUse = ((.hooks.PreToolUse // []) + [$entry])' \
    "$SETTINGS" > "$SETTINGS.tmp" && mv "$SETTINGS.tmp" "$SETTINGS"
else
